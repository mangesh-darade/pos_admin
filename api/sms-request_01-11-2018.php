<?php  
    ...
    Class Sms{
        ...
        public function smsLog($mphone){
        
            $this->marchant_sms_log($mphone);
        }
	   ...
	   if($this->apikey != $apiKey){
        ...
        function marchantDetails($merchant) { 
            $MsgArr = array();
            if (!empty($merchant)) {
        ...
        Private function LogSMS($param){
       
            $m_phone = isset($param['m_phone']) && !empty($param['m_phone'])?$param['m_phone']:'';
            ...
            $t = date("Y-m-d H:i:s");
            $sql = "INSERT INTO  `sitadmin_simplysafe`.`merchant_sms_log` (`merchant_phone` ,`receiver_phone` ,`note`,`sms_id` ,`posttime`,`sms_count`)VALUES ( '$m_phone',  '$receiver_phone',  '$note', '$sms_id', '$t','$sms_count_used')"; 
            if ($result = mysqli_query($this->conn, $sql)) {
        ...
        Private function SendSMS($phone,$note){
             $datasms = array(
                "user" => "simplysafe",
	...
	Private function marchant_sms_log($merchant) { 
	            $MsgArr = array();
    ...
    if (!empty($mphone) && !empty($apikey)):
        ...
        echo json_encode(array('status' => 'error', 'msg' => 'All mendetory field are not supplied1'));
