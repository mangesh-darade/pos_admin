...
function marchantRequest($merchant,$customerName,$customerMobile){   
	...
	if ($rowcount == 1) {
				...
				//echo $url;
				?>
				<script src="http://simplypos.in/api/js/jquery-3.1.1.min.js"></script>
		                <script>
				var merchant  =  '<?php echo $merchant; ?>';
				var customerName =  '<?php echo $customerName; ?>';
				var customerMobile= '<?php echo $customerMobile; ?>';
				var url = '<?php echo $url."adduser.php"; ?>';
				var data = "action=messageInsert&merchant="+merchant+"&customerName="+customerName+"&customerMobile="+customerMobile;
				jQuery.ajax({
				type: "GET",
				data: data,
				url: url,
...
<?php
			...
			if (mysqli_query($conn, $sqlupdate)) {
				...
				var data = "customer_id="+customerMobile+"&merchant_id="+merchant+"&status="+status;
				jQuery.ajax({
				type: "POST",
				data: data,
				url: "http://52.66.101.233/Customer-Backend/public/api/v1/customer/merchant/statusUpdate",
				success: function(data)
				{ 
				docunment.write();
				alert(data);
