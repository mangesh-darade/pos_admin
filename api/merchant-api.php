...
if (!empty($_REQUEST['action'])) {
    ...
    switch ($_REQUEST['action']) {
        ...
        case 'marchantNo':
            $MsgArr = marchantNo($_REQUEST['merchant']);
            break;

        case 'updateStatus':
            $MsgArr = updateStatus($_REQUEST['merchant'], $_REQUEST['customerMobile'], $_REQUEST['status'], $_REQUEST['customerName']);
            break;
...
function marchantRequest($merchant, $customerName, $customerMobile) {
    ...
    if ($rowcount > 0) {
        ...
        $MsgArr['result']  = '0';
        $MsgArr['message'] = 'Already exist';
      
        if(isset($row1["status"]) && !empty($row1["status"])):
            switch ($row1["status"]) {
                case 'accepted':
                        $MsgArr["status"] = 1;
                    break;
        ...
        if (!empty($merchant) && !empty($customerName) && !empty($customerMobile)) {
    	    ...
    	    $rowsql   = mysqli_num_rows($ressql);
    	  
    	    if ($rowsql > 0) {
            $pending = "Pending";
            $sqlInserts = "INSERT INTO `merchant_requests` ( `merchant_code` ,`customer_code` , `customer_name`,`status`)"
            ...
            if (mysqli_query($conn, $sqlInserts)) {
                ...
                $row        = mysqli_fetch_array($result, MYSQLI_ASSOC);
                $MsgArr['result']  = '1';
                $MsgArr['message'] = 'Inserted Successfully';
                $MsgArr["status"]  = 0;
                $MsgArr["res"]     = $row;
                $url               = marchantURL($merchant);
                $url               = $url['pos_url'];
        ...
        }else{ $MsgArr["res"] = "Merchant not exist";}
        }
         else {
            $MsgArr["res"] = "error";
...
function marchantNo($merchant) {
    ...
    $MsgArr = array();
    if (!empty($merchant)) {
        $sqlInsert = "SELECT `phone` FROM `merchants` WHERE `pos_url` LIKE '%$merchant%'";
        //$result = mysqli_query($conn,$sql);
        //$row=mysqli_fetch_array($result,MYSQLI_ASSOC);
        if ($result = mysqli_query($conn, $sqlInsert)) {
            $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
...
//random password//
//Status update
function updateStatus($merchant, $customerMobile, $status,$customerName) {
    global $conn;
    $MsgArr = array();
    if (!empty($merchant) && !empty($customerMobile) && !empty($status) && !empty($customerName)) {
        ...
        $pos_key ='';
            if ($status == "accepted") {
                $status_change = 1;
                $select_url = "SELECT * FROM `merchants` WHERE `phone` = $merchant";
                $result     = mysqli_query($conn, $select_url);
                $row        = mysqli_fetch_array($result, MYSQLI_ASSOC);
                $url        = marchantURL($merchant);
                $url        = $url['pos_url'];
                $MsgArr["url"]     = $url ; 
                ...
                $surl_req   = $url . 'api/v2';
                $res_req    = post_to_url($surl_req, $data_req ); 
                $key = json_decode($res_req);
                $pos_key = $key->pos_key;
            ...
            $surl = "https://simplypos.co.in/api/v1/customer/merchant/statusUpdate";
            $res = post_to_url_json($surl, $data);
	    $logfile = __dir__.'/decline_log.log';
	    file_put_contents($logfile,date("Y-m-d H:i:s")." | ". serialize($data_req)."\n",FILE_APPEND | LOCK_EX);
	    file_put_contents($logfile,date("Y-m-d H:i:s")." | ".  $res_req."\n",FILE_APPEND | LOCK_EX);
...
function post_to_url_json($url, $data) {
