...
if($_REQUEST["step"]==1) {
    ...
    $subdomainResult = $objMerchant->cp_createSubDomain($subDomainName, $hostName, $ProjectDirectory);
            ...
            if($createDB['status'] == 'SUCCESS') {      
                ...
                if($addDbUser['status']=='SUCCESS') 
                {
                    ...
                    if($setPrivileges['status'] == 'SUCCESS') {
                     ...
                     echo '<div class="alert alert-danger">'.$addDbUser['msg'].'</div>';
...
if($_REQUEST['step'] == 3) {
    ...
    // MySQL password
    $mysql_password = $dbData['db_passwd'];
    // Database name
...
if($_REQUEST['step'] == 6){
    ...
    if(checkPathWritable($fname)) :
        ...
        $chd   = array($dbData['db_user'], $dbData['db_passwd'], $dbData['db_name']);
