...
FROM  `sma_users` ";
 ...
 $mailData['body'] = "<div style='font-family:arial; font-size:14px; text-align:left;'>
    ...
    public function getMerchantPosData($merchant_id) {
        
       $sql = "SELECT `id` , `pos_name` , `pos_url` , `db_name` , `db_username` , `db_password` , `pos_current_version` , `sql_current_version`, `project_directory_path`
    ...
    */
    ...
    public function getPosLatestVersion() {
        ...
        $sql = "SELECT `id`, `version`, `relese_status`, `relese_type` , `relese_date`, `upgread_sql`, `sql_file_path_up`, `project_code_path`"
                ...
                WHERE `version`  
                            IN (
                                SELECT MAX( version )  FROM  `pos_versions` 
    ...
    private function __pos_update_step1($merchant_id) {
        ...
        if($data['status'] == 'SUCCESS') {
            ...
            $setlog = $this->__set_pos_backup_log($backupSql, $merchant_id , $type='sql');
    ...
    private function __pos_update_step2($merchant_id) {
        ...
        if($data['status'] == 'SUCCESS') {
            ...
            $setlog = $this->__set_pos_backup_log($backupPos, $merchant_id , $type='code');
    ...
    private function __copy_version_files_to_pos( $posinfo , $versionInfo){            
                ...
                foreach ($files as $key => $filePath) {
                    ...
                    if($avoid === false) {
                        ...
                        if($objopen){
                            ...
                            if (is_dir($path))
                            {
                              echo '<br/>'. $addPath.   var_dump(  $zip->addEmptyDir( $addPath ));            
                            }

                            if (is_file($path))
                            {
                               echo '<br/>'. $addPath.   var_dump(  $zip->addFile($path , $addPath ));
                            } 
        ...
        if(file_exists($versionPath.'/' . _VERSION_UPGRADE_ZIP) === true)
        {
            if($zip->open($versionPath.'/' . _VERSION_UPGRADE_ZIP) === TRUE) 
            {
    ...
    private function __set_pos_backup_log($backupFile, $merchant_id, $type='sql' ) {
        ...
        $sql = "INSERT INTO ".TABLE_MERCHANT_POS_BACKUPS_LOG." (`backup_time`, `merchant_id`, `type`, $fieldname ) "
                    . " VALUES ( CURRENT_TIMESTAMP, '$merchant_id', '$type', '$backupFile')";
    ...
    private function __upgrade_version_database(array $posinfo , array $versionInfo) {
        ...
        $sqlInfo = $this->get_upgrade_pos_sql_list($current_pos_version);
    ...
    public function get_upgrade_pos_sql_list($current_pos_version , $methos='up') {
        ...
        $sql = "SELECT  `id` , `version`, `upgread_sql`, `sql_file_path_up`, `sql_file_path_down` "
                . " FROM ".TABLE_POS_VERSIONS."  WHERE `version` > '$current_pos_version' ORDER BY `version` $orderSys ";
