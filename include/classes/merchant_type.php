...
class merchant_type {
    ...
    public function filter_data(array $resuestData)
    {
        ...
        $developerCondition = " AND `id` NOT IN (999) ";
        ...
        if($resuestData['result_type'] == 'filter') {
            ...
            $sqlCount = "SELECT count(id) as num FROM " . TABLE_VIEW_MERCHANT_TYPE . " WHERE " . $whereClause . " " . $developerCondition ;
            
            $query = "SELECT * FROM " . TABLE_VIEW_MERCHANT_TYPE . " 
                     WHERE " . $whereClause . " " . $developerCondition . " ORDER BY `merchant_type`   
        ...
        if($resuestData['result_type'] == 'search') {
            ...
            $sqlCount = " SELECT count(id) as num FROM ".TABLE_VIEW_MERCHANT_TYPE."  WHERE ( `merchant_type` LIKE  '%$search_key%'  )  
                                     ".$developerCondition;
            
            $query = " SELECT * FROM ".TABLE_VIEW_MERCHANT_TYPE."  WHERE ( `merchant_type` LIKE  '%$search_key%'  )  
                                    ".$developerCondition . 
                                    " ORDER BY `id` DESC  
                                    LIMIT ".( $pageno - 1 ) * $itemsPerPage .", " .$itemsPerPage;
        }
    ...
    public function insert($postData) {
        ...
        $merchant_type = $this->appCommon->prepareInput($merchant_type);
        
        if($this->merchantTypeIsExists($merchant_type , 0) == true){
            $data['status'] ='ERROR';
            $data['msg'] = 'Merchant type already exist.';
            return $data;
        }
        
        $sql = "INSERT INTO " . TABLE_MERCHANT_TYPE . " ( `merchant_type`, `show_in_mobile`, `show_in_web`, `generate_pos`, `is_active`, `pos_project_id`, `pos_sample_data_id` )
             VALUES ('$merchant_type', '$show_in_mobile', '$show_in_web', '$generate_pos', '1' , '$pos_project_id', '$pos_sample_data_id' )";
    ...
    public function merchantTypeIsExists($inpurValue , $id='') {
        ...
        $sql = "SELECT `id` FROM " . TABLE_MERCHANT_TYPE . " WHERE `merchant_type` = '$inpurValue'  $where ";
    ...
    public function update($postData) {
        ...
        if($this->merchantTypeIsExists($merchant_type , $id) == true){
            $data['status'] ='ERROR';
            $data['msg'] = 'Merchant type already exist.';
            return $data;
        }
        
        $sql = "UPDATE " . TABLE_MERCHANT_TYPE . " SET "
                . " `merchant_type` = '$merchant_type', "
                . " `show_in_mobile` = '$show_in_mobile', "
                . " `show_in_web` = '$show_in_web', "
                . " `generate_pos` = '$generate_pos', "
                . " `pos_project_id` = '$pos_project_id', "
                . " `pos_sample_data_id` = '$pos_sample_data_id' "
                . "  WHERE `id` = '$id' ";
