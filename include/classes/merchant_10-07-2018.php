...
FROM  `sma_users` ";
 ...
 $mailData['body'] = "<div style='font-family:arial; font-size:14px; text-align:left;'>
    ...
    public function getMerchantPosData($merchant_id, $select='') {
        ...
        if(empty($select)) {
            $select = " M.`id`, M.`name`, M.`business_name` , M.`type` AS pos_type_id, MT.`keyword` AS pos_type, M.`pos_name`, M.`phone` , M.`pos_url` , M.`db_name` , M.`db_username` , M.`db_password` , M.`pos_current_version` , M.`sql_current_version`, M.`project_directory_path`, M.`pos_status` ";
        } 
