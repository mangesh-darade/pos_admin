...
FROM  `sma_users` ";
 ...
 $mailData['body'] = "<div style='font-family:arial; font-size:14px; text-align:left;'>
    ...
    public function getMerchantPosData($merchant_id) {
        ...
        $sql = "SELECT `id` , `pos_name`, `phone` , `pos_url` , `db_name` , `db_username` , `db_password` , `pos_current_version` , `sql_current_version`, `project_directory_path`, `pos_status`
        ...
        {            
            ...
            if($setSuspend == true) {
               ...
               $sql = "UPDATE  ".TABLE_MERCHANTS."  SET  `pos_previous_status` = `pos_status` , `pos_status` = 'suspended'  WHERE  `id` = '$merchant_id' ";
                ...
                if($result){
                    $this->logUserActivity(['merchant_id'=>$merchant_id , 'activity'=>'Pos suspended.']);
                    return true;
                } else {
                    return false;
                }               
    ...
    */
        ...
        if($zipexists)
        {
            ...
            if($zip->open($versionZipFile) === TRUE) 
            {
				...
				if(is_dir($oldShopDirectory)){
					...
					rename($oldShopDirectory, $oldShopDirectory.'_old');
				}
    ...
    public function pos_rollback_setup($merchant_id , $backup_version_id) {
        ...
        if($isDbDeleted) 
        {
            ...
            if($isDbImported['status']) //$isDbImported
            {               
