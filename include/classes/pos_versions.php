...
class pos_versions {
    ...
    public function __construct() {
        ...
        $this->pos_latest_version = 4.1;
        $this->pos_latest_stable_version = 3.05;
    ...
    public function filter_data(array $resuestData) {
        ...
        if ($resuestData['result_type'] == 'filter') {
            ...
            $query = "SELECT * FROM " . TABLE_POS_VERSIONS . " 
                     WHERE " . $whereClause . " " . $developerCondition . " ORDER BY `relese_date`  DESC   
    ...
    public function insert($postData) {
        ...
        $insert['project_id'] = $this->appCommon->prepareInput($project_id);
        ...
        if ($this->projectVersionIsExists($insert['version'], 0, $insert['merchant_group']) == true) {
            $data['status'] = 'ERROR';
    ...
    public function projectVersionIsExists($inpurValue, $id = '', $project_id='1') {
        ...
        $sql = "SELECT `id` FROM " . TABLE_POS_VERSIONS . " WHERE `version` = '$inpurValue' AND `project_id` = '$project_id'  $where ";
    ...
    public function update($postData) {
        ...
        if ($this->projectVersionIsExists($insert['version'], $id, $insert['merchant_group']) == true) {
            $data['status'] = 'ERROR';
            $data['msg'] = 'Project version alredy exist.';
            return $data;
        }
    ...
    public function getProjectGroups($id = NULL) {
