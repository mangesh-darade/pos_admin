...
class baseClass {
    ...
    public function __construct($conn) {
       ...
       $this->defiendSettings();
        ...
        //Backup Copy of MEK: Do not change this key in any cost.
...
public function SendSMS($mobile,$msg ){
    ...
    $config['smsAPI']='http://login.yoursmsbox.com/api/sendhttp.php';
    ...
    $smsAPI  = $config['smsAPI'];
    $datasms  = "?authkey=". $config['authkey'];
    $datasms .= "&route=". $config['route'];
    $datasms .= "&response=".  $config['response'];
    $datasms .= "&country=". $config['country'];            
    $datasms .= "&sender=". $config['sender'];
    ...
    public function sendMail(array $mailData )    {
        ...
        $mailCC         = $mailData['cc'];
        $mailBCC        = $mailData['bcc'];
        $mailCCName         = $mailData['cc_name'];
        $mailBCCName        = $mailData['bcc_name'];
 ...
 */ 
        ...
        if($mailBCC){
            $mail->addBCC($mailBCC, $mailBCCName);
        }
    ...
    public function randomPasswd($length = 10)     {
        $characters = '~_=&#*!@%+^{?}[;:,<.>]KABCDEFGHJLMNPQRSTUVWYZ12670475938abcdefghjkmnpqrstuvwyz';
...
public function dateDiff($startDate, $endDate) 
{
    ...
    public function export_database($dbCon,  $mysqlDatabaseName , $version='1.0', $backup_path='' )
    {
        ...
        foreach($target_tables_sorts as $table)
        {
    ...
    public function runSqlFile($posconn , $filename) {
        ...
        //Temporary variable, used to store current query
            ...
            foreach ($lines as $line)
            {
                ...
                {
                    $pattern = "/DEFINER=`.*`@`.*` SQL SECURITY DEFINER/";
                    $replacement = '';
    ...
    // Encrypt Function
    public function mc_encrypt($encrypt, $key=MASTER_ENCRYPTION_KEY){
    ...
    // Decrypt Function
    public function mc_decrypt($decrypt, $key=MASTER_ENCRYPTION_KEY){
    ...
    public function setPrivateKeyEncrypt($key){
    
        return $this->mc_encrypt(sha1($key.time()));
