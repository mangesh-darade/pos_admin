...
class apiRequests extends  merchant 
{
	...
	private function MerchantDetail($merchant_mobile) { 
 	    if(strlen($merchant_mobile) != 10) return false;
            if (!empty($merchant_mobile)) {
               $sql = "SELECT * FROM `merchants` WHERE `phone` = '$merchant_mobile' and is_active='1' and  is_delete='0' and subscription_is_active='1' "; 
                if ($result = mysqli_query($this->conn, $sql)) {
                    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
                    if(!isset($row['id'])):
                     return false;
	...
	public function updateSmsCounter($merchant_mobile){
		...
		if(empty($smsPermission) || $smsPermission==0):
		   $return['msg'] = 'Insufficient balance to send SMS';
		   $this->jsonEncode($return);
		elseif($smsPermission > 0):
		   $sql = "update `merchants`   set  `sms_balance`=`sms_balance`-1 , `sms_used`=`sms_used`+1 where `phone` = '$merchant_mobile' "; 
	           if ($result = mysqli_query($this->conn, $sql)) {
	              $return['status'] = 'success' ; 
