...
class auth extends merchant {
    ...
    public function login(){
        ...
        if($this->validLoginDetails())
        {
            ...
            if($isValidUser == false) {
               ...
               $drcpt_password = $this->objapp->mc_decrypt($this->merchantData['password'], MASTER_ENCRYPTION_KEY);
                if( $drcpt_password === $this->password){
    ...
    public function isValidUser() {
        ...
        $sql = "SELECT `id`, `name`, `email`, `phone` , `password_encript` as password , `is_active`, `is_delete` ,  COUNT(`id`) as num 
                FROM ".TABLE_MERCHANTS."
                WHERE ( `email` = '".$this->username."' OR `phone` = '".$this->username."' )   LIMIT 1";
    ...
    public function validateToken($pass_token,$auth_token,$password){
        ...
        $password_encript = $this->objapp->mc_encrypt($password, MASTER_ENCRYPTION_KEY);
        
        $Upsql = "UPDATE `merchants`  SET `password_encript` = '$password_encript', pass_token='' ,pass_token_start_date='' ,pass_token_end_date='' WHERE `id` = '".$row['id']."' ";
