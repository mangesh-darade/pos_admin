...
class merchant {
    ...
    public function merchants_report($post) {
        ...
        $query = "SELECT M.`id`, M.`name`, M.`email`, M.`phone`, M.`type`, M.`distributor_id`, M.`business_name`, M.`pos_name`, M.`pos_url`, M.`pos_current_version`,
            ...
            P.`package_name` , S.`validity_balance` , MG.`group_name`
    ...
    public function getDistributors($id=''){
         $where = '';
        if($id) { $where = " AND user_id = '$id' " ; }
        
        $sql = "SELECT `user_id`,`display_name`, `email_id`,`mobile_no` "
                    . " FROM   " . TABLE_ADMIN
                    . " WHERE `is_disrtibuter` = 1 ".$where
    ...
    public function getMerchantGroup($id=NULL){ 
...
FROM  `sma_users` ";
    ...
    public function pos_settings() {
        ...
        $sql = "SELECT `setting_id`, `pos_version`, `pos_type`, `api_privatekey`, `api_access` 
                FROM  `sma_settings` ";
 ...
 $mailData['body'] = "<div style='font-family:arial; font-size:14px; text-align:left;'>
    ...
    public function suspend_email_send($merchant) {
        ...
        $mailData['subject'] = $merchant['pos_name'] .' POS suspension alert ';
        
        $mailbody = "<p>POS ".$merchant['pos_name']." has been suspeneded.</p><br/><br/>";
    ...
    public function merchant_remove($merchant_id) {
        ...
        if(is_numeric($merchant_id) && $merchant_id > 0 )
        {
            ...
            $sql = "UPDATE ".TABLE_MERCHANTS." SET pos_generate = '0', pos_previous_status = pos_status , pos_status='pending' WHERE `id` = '$merchant_id' ";
...
//                   exit;
	...
	public function getLatestPOSVersion(){
		$sql="SELECT version, relese_status, relese_date FROM `pos_versions` WHERE 1 order by `id` desc limit 0,5";
		$result = $this->conn->query($sql);
		$logData = array();
		while($row = $result->fetch_assoc())
        {
	...
	public function getCurrentPOSVersion(){
		$sql="SELECT version, relese_status, relese_date FROM `pos_versions` WHERE 1 order by `id` desc limit 0,1";
		$result = $this->conn->query($sql);
		$logData = array();
		while($row = $result->fetch_assoc())
        {
	...
	function get_feedback_by_merchant_id($Data, $startpoint, $per_page, $search=''){
	...
	{
		$developerCondition = $this->getMerchantDeveloperCondition();
		$Sql = "SELECT count(*) as num FROM `merchants` WHERE `pos_status` = 'pending' AND `is_delete` = '0' $developerCondition ";
		$Query = $this->conn->query($Sql);
		$result = $Query->fetch_assoc();
		//echo $result['num'];
		return $result['num'];
	...
	{
		$developerCondition = $this->getMerchantDeveloperCondition();
		$Sql = "SELECT count(*) as num FROM `merchants` WHERE `pos_generate` = '1' AND `is_delete` = '0' $developerCondition ";
		$Query = $this->conn->query($Sql);
		$result = $Query->fetch_assoc();
		//echo $result['num'];
		return $result['num'];
