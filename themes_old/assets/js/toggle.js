$(document).ready(function() {
    ...
    if (document.cookie.indexOf('collapsed_Nodes') >= 0) {
        ...
        for (var i = 0; i < arrayLength; i++) {
            ...
            $('[data-node="#' + checkCollapsed[i] + '"]').children('.plusminus').children('img').attr('src', base_url +'themes/assets/icon/plus.svg');
        }
    }
    ...
    $('.accordion_head').click(function() {
        if ($(this).next('.accordion_body').is(':visible')) {
            ...
            $(this).children('.plusminus').children('img').attr('src',  base_url +'themes/assets/icon/plus.svg');
        } else {
            $(this).next('.accordion_body').slideDown(300);
            $(this).next('.accordion_body').removeClass('hide');
            $(this).children('.plusminus').children('img').attr('src',  base_url +'themes/assets/icon/minus.svg');
        }
        adjustCookie();
    });
